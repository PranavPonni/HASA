...
from et_rnn import ExternalTouchRNN
...
class RNN_controller(AbstractController):
    ...
    def train_controller(self, sweep=False):
        ...
        for epoch in range(total_epoch):
            ...
            with torch.no_grad():
                ...
                (total_loss,loss_tactile_idx,loss_tactile_thumb,loss_pos,loss_vel,loss_trq,loss_cmd),\
                (idx_preds,thumb_preds,pos_preds,vel_preds,trq_preds,cmd_preds) = self.calc_loss_func(data,"test")
    ...
    def test_controller(self):
        ...
        print("Device: ", self.device)
        EPOCH=4999
        DIR="/root/motionlearning/data_server/insertion_flatten/0.7_-20_episode30"
        self_touch_param=dp.read_yaml(self.model_param["st_param"])["Model"]
        self.model_param={**self_touch_param,**self.model_param}
        self.selftouch = SelfTouch(self.model_param).to(self.device)
        self.model=ExternalTouchRNN(self.selftouch,self.model_param).to(self.device)
        ...
        for ep, val in dir_data.items():
            ...
            dir_data[ep]["hand_jnt_trq"]=val["hand_jnt_trq"][:,[0, 1, 2, 3, 12, 13, 14, 15]]
            dir_data[ep]["hand_jnt_cmd_pos"]=val["hand_jnt_cmd_pos"][:,[0, 1, 2, 3, 12, 13, 14, 15]]
        
        proc_dir_data = dp.scale_dir_data(dir_data, scaling_param, self.dataset_param, mem="ram")
        proc_dir_data = dp.rearrange(proc_dir_data, self.dataset_param, mem="ram")

        state=None
        self_touch=None
        pred_hand_jnt_cmd_pos_ls=[]
        with torch.no_grad():
            for ts in range(proc_dir_data[EPISODE]["hand_jnt_pos"].shape[0]):
                data = {key: torch.tensor(val)[ts][None,:].to(self.device).float() for key,val in proc_dir_data[EPISODE].items()}

                if ts==0:
                    (idx_ext_pred,thumb_ext_pred,jnt_pos_pred,jnt_vel_pred,jnt_trq_pred,jnt_cmd_pos_pred,total_idx_pred,total_thumb_pred),\
                        self_touch, state \
                        = self.model.forward(data["tactile_index_tip"],data["tactile_thumb_tip"],\
                                                data["hand_jnt_pos"],data["hand_jnt_vel"],data["hand_jnt_trq"],\
                                                self_touch,state)
                else:
                    (idx_ext_pred,thumb_ext_pred,jnt_pos_pred,jnt_vel_pred,jnt_trq_pred,jnt_cmd_pos_pred,total_idx_pred,total_thumb_pred),\
                        self_touch, state \
                        = self.model.forward(total_idx_pred,total_thumb_pred,\
                                                jnt_pos_pred,jnt_vel_pred,jnt_trq_pred,\
                                                self_touch,state)
                pred = {}
                pred[EPISODE] = {}
                ...
                pred_hand_jnt_cmd_pos_ls+=[pred[EPISODE]["hand_jnt_cmd_pos"]]
        
        graph_to_video = vis.GraphToVideo()
        graph_to_video.add_graph({"hand_jnt_cmd_pos": dir_data[EPISODE]["hand_jnt_cmd_pos"],\
                                  "pred_hand_jnt_cmd_pos": np.concatenate(pred_hand_jnt_cmd_pos_ls,axis=0)})
    ...
    def motion_controller(self):
        ...
        with torch.no_grad():
            for ts in range(250):
                ...
                (_,_,_,_,_,jnt_cmd_pos_pred,_,_),\
                    self_touch, state \
                    = self.model.forward(data["tactile_index_tip"],data["tactile_thumb_tip"],\
                                            data["hand_jnt_pos"],data["hand_jnt_vel"],data["hand_jnt_trq"],\
                                            self_touch,state)
