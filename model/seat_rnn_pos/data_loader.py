...
class CustomDataLoader(BaseDataLoader):
    def __init__(self, dataset_param,model_param):
        ...
        for ep, val in data.items():
            val["tactile_index_self"] = val["tactile_index_tip"] - val.get("tactile_index_ext", np.zeros_like(val["tactile_index_tip"]))
            val["tactile_thumb_self"] = val["tactile_thumb_tip"] - val.get("tactile_thumb_ext", np.zeros_like(val["tactile_thumb_tip"]))
