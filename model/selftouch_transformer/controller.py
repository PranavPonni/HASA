...
class RNN_controller(AbstractController):
    ...
    def train_controller(self, sweep=False):
        ...
        for epoch in range(total_epoch):
            ...
            with torch.no_grad():
                ...
                li  = float(loss_idx.detach().cpu())
                lt  = float(loss_thb.detach().cpu())
                lm  = float(loss_mid.detach().cpu())
                bl  = float(best_total_loss.detach().cpu()) \
                    if isinstance(best_total_loss, torch.Tensor) else float(best_total_loss)
    ...
    def calc_loss_func(self, data, mode="train"):
        ...
        with autocast(getattr(self, "use_amp", False)):
            (total_loss, loss_idx, loss_thb, loss_mid), (idx_pred, thumb_pred, middle_pred) = \
                self.model.forward_loss(**data, loss_coef=self.loss_coef)
        ...
        return (total_loss, loss_idx, loss_thb, loss_mid), (idx_pred, thumb_pred, middle_pred)
