...
from set_rnn import ExternalTouchRNN
...
class RNN_controller(AbstractController):
    ...
    def test_controller(self):
        ...
        pred_self_idx, pred_self_thumb, pred_ext_idx, pred_ext_thumb = [], [], [], []
        for ts in range(proc_data["motion"]["hand_jnt_pos"].shape[0]):
            sample = {k: torch.tensor(v[ts][None]).float().to(self.device) for k, v in proc_data["motion"].items()}
            idx_s, thumb_s, idx_e, thumb_e = self.model(
                sample["tactile_index_tip"],
                sample["tactile_thumb_tip"],
                sample["hand_jnt_pos"]
            )[0]
            pred_self_idx.append(idx_s.squeeze(0).detach().cpu().numpy())
            pred_self_thumb.append(thumb_s.squeeze(0).detach().cpu().numpy())
            pred_ext_idx.append(idx_e.squeeze(0).detach().cpu().numpy())
            pred_ext_thumb.append(thumb_e.squeeze(0).detach().cpu().numpy())

        def unscale(x, name): return dp.unscale_data(x, scaling_param[name], self.dataset_param["modality"][name])
        gt_idx = unscale(dir_data["motion"]["tactile_index_tip"], "tactile_index_tip")
        gt_thumb = unscale(dir_data["motion"]["tactile_thumb_tip"], "tactile_thumb_tip")
        pred_self_idx = unscale(np.stack(pred_self_idx), "tactile_index_tip")
        pred_self_thumb = unscale(np.stack(pred_self_thumb), "tactile_thumb_tip")
        ...
        plot(norm(gt_idx), norm(pred_self_idx), "GT", "Self", "SET-RNN: Index - Self Touch", "./log/set_index_self.png")
        plot(norm(gt_thumb), norm(pred_self_thumb), "GT", "Self", "SET-RNN: Thumb - Self Touch", "./log/set_thumb_self.png")
        plot(norm(gt_idx), norm(pred_ext_idx), "GT", "External", "SET-RNN: Index - External", "./log/set_index_ext.png")
        plot(norm(gt_thumb), norm(pred_ext_thumb), "GT", "External", "SET-RNN: Thumb - External", "./log/set_thumb_ext.png")

        pd.DataFrame({
            "Timestep": t,
            "GT_Index": norm(gt_idx),
            "Self_Index": norm(pred_self_idx),
            "External_Index": norm(pred_ext_idx),
            "GT_Thumb": norm(gt_thumb),
            "Self_Thumb": norm(pred_self_thumb),
            "External_Thumb": norm(pred_ext_thumb),
        }).to_csv("./log/set_timeseries.csv", index=False)
