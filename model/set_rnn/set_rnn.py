...
class ExternalTouchRNN(nn.Module):
    def __init__(self, selftouch: nn.Module, param: dict):
        ...
        rec_in = 4 * self.tactile_dim + 3 * self.hand_dim
    ...
    def forward(
        ...
        if prev_self_touch is None:
            seq_args = [hand_jnt_pos_t,
                        hand_jnt_vel_t,
                        hand_jnt_trq_t,
                        hand_jnt_pos_t]
            idx_self_t, thumb_self_t, _, _, _ = self.selftouch(*seq_args)
        else:
        ...
        seq_args_next = [jnt_pos_pred,
                         jnt_vel_pred,
                         jnt_trq_pred,
                         jnt_cmd_pos_pred]
        idx_self_next, thumb_self_next, _, _, _ = self.selftouch(*seq_args_next)
    ...
    def forward_loss(
        ...
        for t in range(T-1):
            ...
            preds, (idx_self_next, thumb_self_next), hidden = self.forward(
                tin_t, tth_t, pos_t, vel_t, trq_t, prev_self_touch, hidden
            )
            (_, _, pos_pred, vel_pred, trq_pred, cmd_pred,
             total_idx_pred, total_thumb_pred) = preds
