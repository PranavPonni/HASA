...
export SELFTOUCH_EVAL_MICRO_BATCH_SIZE="${SELFTOUCH_EVAL_MICRO_BATCH_SIZE:-1}"
...
for variant in "${variants[@]}"; do
  ...
  if [ ! -f "$param_file" ]; then
    echo "Missing parameter file for ${variant}: ${param_file}" >&2
    status=1
    continue
  fi
  ...
  echo "Starting ${variant}; param: ${param_file}; log: ${log_path}"
  (
